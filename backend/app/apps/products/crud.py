...
class ProductManager:
    async def create_product(self, session, title, price, description, uuid_id, main_image, images, category_id=None,
                             stock=0, discount_price=None):
...
class CartManager:
    ...
    async def get_or_create_cart(self, session, user_id):
        query = select(Cart).where(Cart.user_id == user_id).options(
            selectinload(Cart.cart_items).selectinload(CartItem.product).selectinload(Product.category)
        )
        result = await session.execute(query)
        cart = result.scalar()
