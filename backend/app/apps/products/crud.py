...
from backend.app.apps.products.models import Product, Category, Cart, CartItem, Order, OrderItem
from backend.app.apps.users.schemas import RegisterUserSchema
...
class ProductManager:
    async def create_product(self, session, title, price, description, uuid_id, main_image, images, category_id=None, stock=0, discount_price=None):
    ...
    async def get_product(self, session, product_uuid: str) -> Product | None:
        query = select(Product).filter(Product.uuid_id == product_uuid).options(selectinload(Product.category))
        result = await session.execute(query)
        return result.scalar_one_or_none()

    async def get_products(self, session, q: str, category_id: int = None) -> list[Product]:
...
class CategoryManager:
    ...
    async def get_category_by_slug(self, session, slug):
...
class OrderManager:
    ...
    async def get_order(self, session, order_id, user_id):
