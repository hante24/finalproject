...
async def login_action(request: Request, username: str = Form(), password: str = Form()):
    async with httpx.AsyncClient() as client:
        response = await client.post("http://backend:8000/users/login", data={"username": username, "password": password})
    if response.status_code == 200:
...
async def index(request: Request, user: dict = Depends(get_user)):
    ...
    context = {
        ...
        "cart_count": await get_cart_count(request)
    }
...
async def catalog(request: Request, q: str = "", category_id: str = None, user: dict = Depends(get_user)):
    ...
    async with httpx.AsyncClient() as client:
        ...
        params = {'q': q}
        if valid_cat_id:
            params['category_id'] = valid_cat_id
...
async def product_detail(request: Request, product_id: int, user: dict = Depends(get_user)):
    ...
    context = {
        "request": request, "user": user,
        "product": product, "cart_count": await get_cart_count(request)
    }
...
async def cart_page(request: Request, user: dict = Depends(get_user)):
    ...
    total = sum(
        (item['product'].get('discount_price') or item['product'].get('price', 0)) * item['quantity']
        for item in items
    )
    context = {
        "request": request, "user": user,
        "cart_items": items, "total_price": total,
...
async def checkout_action(
    ...
    async with httpx.AsyncClient() as client:
        ...
        await client.post('http://backend:8000/products/orders/create', headers=headers, data=payload)
    return RedirectResponse('/orders', status_code=status.HTTP_303_SEE_OTHER)


@router.get("/orders")
async def orders_page(request: Request, user: dict = Depends(get_user)):
    if not user:
        return RedirectResponse('/login', status_code=status.HTTP_303_SEE_OTHER)
    access_token = request.cookies.get('access_token')
    async with httpx.AsyncClient() as client:
        headers = {'Authorization': f"Bearer {access_token}"}
        res = await client.get('http://backend:8000/products/orders/my', headers=headers)
        orders = res.json() if res.status_code == 200 else []

    context = {
        "request": request, "user": user,
        "orders": orders, "cart_count": await get_cart_count(request)
    }
    return templates.TemplateResponse(request=request, name='pages/orders.html', context=context)
